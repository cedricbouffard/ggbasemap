...
# Create the map with clipping for clean edges
ggplot() +
  add_basemap(nc, 
              bbox = rot$bbox,
              clip = rot$clip_poly,
...
### Using Clipping for Clean Edges

When rotating maps, you can use the `clip` and `coord_crs` parameters to ensure clean, straight edges that match the rotated view:

```{r clipping-example, fig.cap="Clean edges with clipping"}
rot <- coord_rotate(nc, angle = 45, ratio = 16/9)

ggplot() +
  add_basemap(nc, 
              bbox = rot$bbox,
              clip = rot$clip_poly,      # Clip to rotated extent
              coord_crs = rot$crs,        # Use rotated projection
              zoom = 8) +
  geom_sf(data = nc, fill = NA, color = "red", linewidth = 1) +
  rot$coord +
  ggtitle("Rotated map with clean clipped edges")
```

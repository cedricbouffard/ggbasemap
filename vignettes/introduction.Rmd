...
knitr::opts_chunk$set(
  ...
  fig.height = 6,
  out.width = "100%",
  eval = FALSE  # Set to TRUE locally to see figures, FALSE for CRAN
)
```
...
**ggbasemap** is an R package that makes it easy to add basemap tiles from XYZ tile servers (like OpenStreetMap, Google, ESRI) to ggplot2 plots. It automatically detects bounding boxes, calculates appropriate zoom levels, and provides powerful image styling options.
...
```{r basic-example, eval=TRUE, fig.cap="Basic map with OpenStreetMap basemap"}
...
```{r df-example, fig.cap="Map with data frame coordinates"}
df <- data.frame(
...
```{r tiles-osm, fig.cap="OpenStreetMap tiles (default)"}
ggplot() +
  add_basemap(nc) +
...
```{r pattern1, fig.cap="Simple basemap"}
ggplot() +
  add_basemap(nc) +
  geom_sf(data = nc)
```
...
```{r pattern2, fig.cap="Styled basemap with transparency"}
ggplot() +
...
```{r pattern3, fig.cap="Grayscale basemap"}
ggplot() +
...
```{r large-area, fig.cap="Large area with lower zoom"}
ggplot() +
...
```{r layers, fig.cap="Multiple layers with basemap"}
ggplot() +
  # Base layer: basemap
...
```{r sharp, fig.cap="High zoom for sharp images"}
ggplot() +
